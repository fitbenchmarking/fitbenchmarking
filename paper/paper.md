...
specifically to interface directly with optimization  packages and individual scientific software packages to test different
implementations of fitting algorithms.  Below we highlight packages available that
are closest to `FitBenchmarking` in spirit.

* `CUTEst` [@cutest] offers a way to benchmark any problems
...
with a set of solvers as a csv file, and produces an analysis of these.
...
* `Benchopt` [@benchopt] is a package which
is primarily used to benchmark data fitting using a range of cost functions.

We designed `FitBenchmarking` to benchmark fitting algorithms applied to data fitting problems, and algorithms as implemented within optimization software packages or within individual scientific software packages.
