...
4. If a new attribute has been added to the ``FittingResult`` class which could be 
   ...
   in ``fitbenchmarking.results_processing.compare_scatter.CompareScatterDataModel.get_plottable_attributes``
   and add a human readable name to the mappings dictionary at 
   ``fitbenchmarking.results_processing.compare_scatter.CompareScatterDataModel._known_mappings``
