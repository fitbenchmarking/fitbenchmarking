...
class TestFittingProblem(TestCase):
    ...
    def test_verify_problem(self):
        ...
        with self.assertRaises(exceptions.FittingProblemError):
            self.fitting_problem.verify()
            self.fail('verify() passes when no values are set.')

        self.fitting_problem.starting_values = [
    ...
    def test_eval_model_raise_error(self):
        ...
        self.assertRaises(exceptions.FittingProblemError,
                          self.fitting_problem.eval_model,
                          x=2,
                          params=[1, 2, 3])
    ...
    def test_eval_model_correct_evaluation(self):
        ...
        self.fitting_problem.function = lambda x, p1: x + p1
        x_val = np.array([1, 8, 11])
        eval_result = self.fitting_problem.eval_model(x=x_val,
                                                 params=[5])
        self.assertTrue(all(eval_result == np.array([6, 13, 16])))

        self.fitting_problem.data_x = np.array([20, 21, 22])
        eval_result = self.fitting_problem.eval_model(params=[5])
    ...
    def test_get_function_params(self):
        ...
        function_def = self.fitting_problem.get_function_params(params=params)
    ...
    def test_set_value_ranges(self):
        ...
        """
        self.fitting_problem.starting_values = [OrderedDict(
            [('param1', 0), ('param2', 0), ('param3', 0)])]
        value_ranges_prob_def = {'param1': (0, 5), 'param2': (5, 10)}
        expected_value_ranges = [(0, 5), (5, 10), (-np.inf, np.inf)]
        self.fitting_problem.set_value_ranges(value_ranges_prob_def)
        self.assertEqual(self.fitting_problem.value_ranges, expected_value_ranges)
    ...
    def test_set_value_ranges_incorrect_names(self):
        ...
        self.assertRaises(exceptions.IncorrectBoundsError,
                          self.fitting_problem.set_value_ranges,
    ...
    def test_correct_data_single_fit(self):
        ...
        self.fitting_problem.data_x = x_data
        self.fitting_problem.data_y = y_data
        self.fitting_problem.data_e = e_data
        self.fitting_problem.start_x = start_x
        self.fitting_problem.end_x = end_x

        self.fitting_problem.correct_data()
        self.assertTrue(
            np.isclose(self.fitting_problem.data_x[
                           self.fitting_problem.sorted_index],
                       expected_x_data).all())
        self.assertTrue(
            np.isclose(self.fitting_problem.data_y[
                           self.fitting_problem.sorted_index],
                       expected_y_data).all())
        self.assertTrue(
            np.isclose(self.fitting_problem.data_e[
                           self.fitting_problem.sorted_index],
                       expected_e_data).all())
        ...
        self.assertTrue(
            np.isclose(self.fitting_problem.data_y[
                           self.fitting_problem.sorted_index],
                       expected_y_data).all())
        self.assertIs(self.fitting_problem.data_e, None)

    def test_correct_data_multi_fit(self):
        """
        Tests correct data on a multifit problem.
        """
        self.fitting_problem.multifit = True
        x_data = [np.array([-0.5, 0.0, 1.0, 0.5, 1.5, 2.0, 2.5, 3.0, 4.0]),
        ...
        for i in range(3):
            self.assertTrue(
                np.isclose(
                    self.fitting_problem.data_x[i][
                        self.fitting_problem.sorted_index[i]],
                    expected_x_data[i]).all())
            self.assertTrue(
                np.isclose(
                    self.fitting_problem.data_y[i][
                        self.fitting_problem.sorted_index[i]],
                    expected_y_data[i]).all())
            self.assertTrue(
                np.isclose(
                    self.fitting_problem.data_e[i][
                        self.fitting_problem.sorted_index[i]],
            ...
            self.assertTrue(
                np.isclose(
                    self.fitting_problem.data_y[i][
                        self.fitting_problem.sorted_index[i]],
                    expected_y_data[i]).all())
            self.assertIs(self.fitting_problem.data_e[i], None)
