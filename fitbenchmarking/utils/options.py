...
class Options:
    ...
    def __init__(self, file_name=None, additional_options=None):
        ...
        config = configparser.ConfigParser(
            converters={"list": read_list, "str": str, "rng": read_range},
            allow_no_value=True,
        )
        ...
        for key in self.VALID_FITTING["software"]:
            self._minimizers[key] = self.read_value(
                minimizers.getlist, key, additional_options
            )
        ...
        self.hes_method = self.read_value(
            fitting.getlist, "hes_method", additional_options
        )

        self.cost_func_type = self.read_value(
            fitting.getlist, "cost_func_type", additional_options
        )

        self.max_runtime = self.read_value(
            fitting.getfloat, "max_runtime", additional_options
        )

        jacobian = config["JACOBIAN"]
        self.jac_num_method = {}
        for key in self.VALID_FITTING["jac_method"]:
            self.jac_num_method[key] = self.read_value(
                jacobian.getlist, key, additional_options
            )

        hessian = config["HESSIAN"]
        self.hes_num_method = {}
        for key in self.VALID_FITTING["hes_method"]:
            self.hes_num_method[key] = self.read_value(
                hessian.getlist, key, additional_options
            )
        ...
        self.comparison_mode = self.read_value(
            output.getstr, "comparison_mode", additional_options
        )

        self.table_type = self.read_value(
            output.getlist, "table_type", additional_options
        )

        self.results_dir = self.read_value(
            output.getstr, "results_dir", additional_options
        )
        ...
        self.runtime_metric = self.read_value(
            runtime.getstr, "runtime_metric", additional_options
        )
    ...
    def _create_config(self):
        ...
        config["MINIMIZERS"] = {
            k: list_to_string(m) for k, m in self.minimizers.items()
        }
        config["FITTING"] = {
        ...
        config["JACOBIAN"] = {
            k: list_to_string(m) for k, m in self.jac_num_method.items()
        }
        config["HESSIAN"] = {
            k: list_to_string(m) for k, m in self.hes_num_method.items()
        }
...
def read_range(s):
    ...
    if rng[0] < 0 or rng[0] > 1 or rng[1] < 0 or rng[1] > 1:
        raise ValueError(
            "One or more elements in range are "
...
def find_options_file(options_file: str, additional_options: dict) -> Options:
    ...
    if options_file != "":
        ...
        return Options(
            file_name=glob_options_file, additional_options=additional_options
        )
