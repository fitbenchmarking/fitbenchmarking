"""
Methods that prepare the muon problems function definitions to be
in the right format.
"""
...
def muon_func_definitions(functions_string):
    """
    Processing the muon function definition into an appropriate format
    ...
    function_names = get_all_muon_func_names(functions_string)
    function_params = get_all_muon_func_params(functions_string)
    params, ties = get_muon_initial_params_values(function_params)
    fit_function = None
    for name in function_names:
        fit_function = make_muon_fit_function(name, fit_function)
...
def get_all_muon_func_names(functions_string):
    """
    ...
    for function in functions:
        function_names = get_muon_func_names(function, function_names)

    return function_names

def get_all_muon_func_params(functions_string):
    """
    ...
    function_params = []
    for function in functions:
        function_params = get_muon_func_params(function, function_params)

    return function_params

def get_muon_func_names(function, function_names):
    """
...
def get_muon_func_params(function, function_params):
    """
...
def get_muon_initial_params_values(function_params):
    """
    ...
    for param_set in function_params:
        get_muon_params(param_set, params)
        get_muon_ties(param_set, ties)

    params = np.array(params)
    return params, ties

def make_muon_fit_function(func_name, fit_function):
    """
    Create the muon fit function object that is used by scipy.
    """
...
def get_muon_params(param_set, params):
    """
    Get the muon param values from the param_set string array which
...
def get_muon_ties(param_set, ties):
    """
    Gets the muon problem tie values.
    """
