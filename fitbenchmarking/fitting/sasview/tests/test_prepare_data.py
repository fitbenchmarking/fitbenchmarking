...
from fitbenchmarking.fitting.sasview.prepare_data import misc_preparations
from fitbenchmarking.fitting.sasview.prepare_data import apply_x_data_range
...
class SasViewTests(unittest.TestCase):
    ...
    def test_prepareData_use_errors_true(self):
        ...
        np.testing.assert_array_equal(expected_data[0], data.x)
        np.testing.assert_array_equal(expected_data[1], data.y)
        np.testing.assert_array_equal(expected_data[2], data.dy)
